...
> **One deviation from the standard layout, on purpose.** The `doc` kind lives in
> **`kb-docs/`** here, not `docs/`. This repo is an app codebase before it is a knowledge
> base, and `docs/` already means something specific and load-bearing: the ADRs
> (`docs/adr/`) and `docs/HANDOFF.md`, which `CLAUDE.md`'s "read before changing" table
> points straight at. Two schemas for one folder would make both untrustworthy. Everything
> else follows the model unchanged.
>
> Note the consequence: `docs/adr/` is **not** part of this knowledge base. ADRs stay where
> they are and keep their own convention. A `kb-docs/` doc may link to one, but a design
> decision about how content is obtained or interpreted still belongs in an ADR.
...
| `doc` | durable knowledge: an analysis, a decision, a thing you learned | `kb-docs/` | `domain[], status?, links` |
...
| I want to… | Go to |
|---|---|
| record a fact / insight we learned | `kb-docs/` |
...
| read a deep analysis | `kb-docs/` |
| see why we chose something | `kb-docs/` (a decision) |
